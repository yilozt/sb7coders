...
impl Application for MyApplication {
  ...
  fn startup(&mut self) {
    ...
    let gs_source = CString::new(
      "
      #version 460 core

      layout (triangles) in;                  // 输入
      layout (points, max_vertices = 3) out;  // 输出

      void main(void) {
        int i;

        for (i = 0; i < gl_in.length(); i++) {
          gl_Position = gl_in[i].gl_Position; // 输出位置
          EmitVertex(); // 输出顶点
        }
      }
      ",
    ...
    unsafe {
      ...
      gl::ShaderSource(gs, 1, &gs_source.as_ptr(), null());
      gl::CompileShader(gs);
